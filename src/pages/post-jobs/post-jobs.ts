...
export class PostJobsPage implements OnInit {
  ...
  job_sub_category_list: any = [];
  job_available_sub_category_list: any = [];
  show_job_available_subcategory = false;
  disable = false;
  images: any = [];
  form: FormGroup;
  categroyArray: any = [];
  ...
  // }
  getSubCategories(id) {
    this.cService.get("subcategory-afields/" + id).subscribe(
      res => {
        if (res.status && res.data) {
          console.log(res.data.categories);
          this.job_sub_category_list = res.data.categories;
        } else {
          console.log("error getting sub categoories");
        }
      },
      err => {
        console.log("error getting sub category");
      }
    );
  }
      ...
      this.cService.postData(data, "item/s").subscribe(
        res => {
          if (res.status) {
            ...
            // this.toast
            //   .show(
            //     ` classified has been posted Succesfully..Your Item has been posted Succesfully.. Wait till admin approves it`,
            //     "2000",
            //     "center"
            //   )
            //   .subscribe(toast => {
  ...
  chooseSubcategory(item) {
    console.log("choose_subcategoryitem", item);

    if (item.s_name == "Jobs Available" || item.s_name == "المهن متوفرة") {
      this.show_job_available_subcategory = true;
      let id = item.pk_i_id;
      this.cService.get("subcategories", { category_id: id }).subscribe(res => {
        if (Object.keys(res.data).length > 0) {
          this.job_available_sub_category_list = res.data;
          console.log(this.job_available_sub_category_list);
          this.show_job_available_subcategory = true;
        } else {
          console.log("error");
          this.show_job_available_subcategory = false;
        }
      });
    } else {
      console.log("error----", item.s_name);
      this.show_job_available_subcategory = false;
    }
  }
